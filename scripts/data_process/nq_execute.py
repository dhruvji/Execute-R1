...
After reasoning, you should write your entire python solution file with your function signature {function} between <execute> and </execute> and the execution engine will return the python program output between <output> and </output>. \
You can execute new programs as many times as you want. Do not write your own tests. \
If you are finished execution testing, you can directly provide the final code solution inside <answer> and </answer>.""" + """ For example, for a question on writing a function to find the similar elements from the given two tuple lists with the function signature def similar_elements(test_tup1, test_tup2), the answer would be <answer> def similar_elements(test_tup1, test_tup2):\r\n  res = tuple(set(test_tup1) & set(test_tup2))\r\n  return (res)  </answer>.""" + f""" Question: {question}\r\n Your code should pass these tests: <tests> {tests} </tests> """
    else:
...
if __name__ == '__main__':
    ...
    def make_map_fn(split):
        def process_fn(example, idx):
            ...
            function = example['functionSignature']
            # if question[-1] != '?':
            #     question += '?'
            
            question_data = {'question': question, 'test_list': test_list, 'functionSignature': function}
